...
def dummy():
    return
class AbsenceConfigMenu:
    def __init__(self):
        self.title = "Absence Grading Config Menu"
        
        self.menuList = {
             "1": ("Month Grade", lambda: self.bulkView("absence-M"))
        }
    ...
    def bulkView(self, key_name):
        ...
        print("Hint: Lower value is the best Score")
    ...
    def getUserInput (self, key, prev, prevKey):
        while True:
            u_input = input(f"{key}: ")
            try:
                u_input = int(u_input)
                if u_input < 0:
                    ...
                    print(f"cannot be same or under than {prevKey} value ({prev})")
                else:
                    return u_input
            except ValueError:
    ...
    def bulkEdit(self, key_name):
        ...
        for key, _ in target.items():
            user_input = self.getUserInput(key=key, prev=prevVal, prevKey=prevkey)
